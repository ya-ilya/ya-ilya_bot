...
module.exports.run = async (client, msg, args) => {
    ...
    if (!args) {
        ...
        .addField("Usage:", '`!add-money <user> <amount>`', false)
        .setColor(0xff0000)
        await msg.channel.send(embed)
    }else {
        if (args[1] > 0) {
        let amount = args[1]
        db.add(`money_${msg.guild.id}_${ment.id}`, amount);
        let embed = new Discord.MessageEmbed()
        .setAuthor(`${msg.author.tag}`, `${msg.author.displayAvatarURL()}`)
        .setDescription(`${amount} :dollar: was added to the user ${ment.displayName}`)
        .setColor(0x32d160)
        await msg.channel.send(embed)
        }else {
            let embed = new Discord.MessageEmbed()
